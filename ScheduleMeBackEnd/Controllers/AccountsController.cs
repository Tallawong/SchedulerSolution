...
namespace WebApi.Controllers
{
    ...
    public class AccountsController : BaseController
    {
        ...
        public async Task<IActionResult> AuthenticateWithMfa(AuthenticateRequest model)
        {
            try
            {
                log.InfoFormat("Authenticating user with MFA {0} for ipaddress: {1}",
                    model.Email,
                    ipAddress());
                
                var response = await _accountService.AuthenticateWithMfaAsync(model, ipAddress());
            ...
            {
                log.Error($"Error in AuthenticateWithMfa for {model.Email}", ex);
        ...
        public async Task<IActionResult> VerifyMfa(VerifyMfaRequest model)
        {
            try
            {
                log.InfoFormat("Verifying MFA for user {0}", model.Email);
                
                var response = await _accountService.VerifyMfaAsync(model, ipAddress());
                setTokenCookie(response.RefreshToken);

                log.InfoFormat("MFA verification successful for user {0}", model.Email);
                return Ok(response);
            }
            catch (Exception ex)
            {
                log.Error($"Error in VerifyMfa for {model.Email}", ex);
                return BadRequest(new { message = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> EnableMfa(string id, [FromBody] EnableMfaRequest model)
        {
            try
            {
                // users can enable MFA for their own account only
                if (id != Account.Id)
                    return Unauthorized(new { message = "Unauthorized" });

                await _accountService.EnableMfaAsync(id, model.PhoneNumber);
                
                log.InfoFormat("MFA enabled for user {0}", id);
                return Ok(new { message = "MFA enabled successfully. You will receive SMS verification codes for future logins." });
            }
            catch (Exception ex)
            {
                log.Error($"Error enabling MFA for user {id}", ex);
                return BadRequest(new { message = ex.Message });
            }
        }

        [Authorize]
        [HttpPost("{id}/disable-mfa")]
        public async Task<IActionResult> DisableMfa(string id)
        {
            try
            {
                // users can disable MFA for their own account only
                if (id != Account.Id)
                    return Unauthorized(new { message = "Unauthorized" });

                await _accountService.DisableMfaAsync(id);
                
                log.InfoFormat("MFA disabled for user {0}", id);
                return Ok(new { message = "MFA disabled successfully." });
            }
            catch (Exception ex)
            {
                log.Error($"Error disabling MFA for user {id}", ex);
                return BadRequest(new { message = ex.Message });
            }
        }

        [Authorize]
        [HttpGet("{id}/mfa-status")]
        public IActionResult GetMfaStatus(string id)
        {
            try
            {
                // users can check their own MFA status only
                if (id != Account.Id && Account.Role != Role.Admin)
                    return Unauthorized(new { message = "Unauthorized" });

                var mfaEnabled = _accountService.IsMfaEnabled(id);
                
                return Ok(new { mfaEnabled = mfaEnabled });
            }
            catch (Exception ex)
            {
                log.Error($"Error getting MFA status for user {id}", ex);
                return BadRequest(new { message = ex.Message });
            }
        }
