...
import { TimeSlotsTasks } from 'src/app/entities/timeslotstasks';
import { TimeSlotsTasksDTO } from 'src/app/entities/timeslotstasksDTO';
...
export class TimeSlotTasksEditorComponent implements OnInit, AfterViewInit {
  ...
  private refreshTimeSlotsTasks(selectRow: TimeSlotsTasks | null) {
    this.timeSlots.set([]);
    let selectedRow: TimeSlotsTasks | undefined;
    this.accountService
      .getTimeSlotsTasks()
      .pipe(first())
      .subscribe({
        next: (value: TimeSlotsTasksDTO[]) => {
          value.forEach((element: TimeSlotsTasksDTO) => {
            ...
            if (selectRow != null && selectRow.date == slot.date) selectedRow = slot;
          });
        ...
        complete: () => {
          ...
          if (selectedRow != null) this.selectRow(selectedRow);
        },
        error: (error) => {
          this.alertService.error(error);
        },
  ...
  onDeleteTimeSlotTasks(event: MouseEvent, tasks: TimeSlotsTasks) {
    ...
    modalRef.componentInstance.bodyQuestionStr =
      'Are you sure you want to delete time slot profile?';
    ...
    modalRef.result
      .then((data) => {
        ...
        this.accountService
          .deleteTimeSlotsTasks(timeslotsTasks)
          .pipe(first())
          .subscribe({
            next: () => {},
            complete: () => {
  ...
  get getDayStrFromDate(): string {
    return TimeHandler.getDayStrFromDate(
      toDateTime(this.f['scheduledDate'].value).toFormat(this.dateTimeFormat),
    );
  }
