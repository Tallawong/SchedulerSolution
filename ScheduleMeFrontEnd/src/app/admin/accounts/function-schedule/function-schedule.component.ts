...
export class FunctionScheduleComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  users = signal<User[]>([]);
  selectedUser4Function: User | undefined;
    ...
    this.accountService
      ...
      .subscribe({
        next: (groupTasks: AgentTaskConfig[]) => {
          ...
          const atc = this.groupTasks.find((tcs) => {
            return tcs.agentTaskStr == this.functionStr;
          });
  ...
  private refreshAccounts(callback: () => void) {
    ...
    this.accountService
      .getAll() //getSchedules4Date(this.dateTimeStr) //getAll()
      .pipe(first())
      .subscribe({
  ...
  onAddSchedule(event: MouseEvent, button: any) {
    ...
    if (!selectedUser) {
      this.alertService.warn('Select a user before adding a schedule.');
      return;
    }
    ...
    var existing: User[] = this.users().filter((u) => {
      return (
        ...
        u.scheduleGroup == selectedUser.scheduleGroup &&
        u.email == selectedUser.email
      );
    });
    console.assert(
      existing.length <= 1,
      'We have double ' + selectedUser.firstName + ' user for the same user: ' + selectedUser.email,
    );
    if (existing.length > 0) {
      this.alertService.warn(
        selectedUser.email +
          ...
          (existing[0].scheduleGroup.length > 0
            ? ' for group agent ' + existing[0].scheduleGroup
            : ''),
      );
  ...
  onDeleteSchedule(event: MouseEvent, user: User) {
  ...
  isSameUser(u1: User, u2: User): boolean {
    ...
    return (
      u1.date == u2.date &&
      u1.function == u2.function &&
      u1.scheduleGroup == u2.scheduleGroup &&
      u1.email == u2.email
    );
  }
