...
export class GenerateSchedulesComponent implements OnInit, AfterViewInit {
  ...
  static functions2SchedulesMap: Map<FunctionScheduleComponent, User[]> = new Map<
  ...
  ngAfterViewInit(): void {
    this.functionComponents.changes.subscribe((comps: QueryList<FunctionScheduleComponent>) => {
      ...
      newMap.forEach((users, key) => {
        console.log(key);
        this.fComponents.forEach((element) => {
          if (element.functionStr === key.functionStr) {
            GenerateSchedulesComponent.functions2SchedulesMap.set(element, users);
          }
        });
      });
  ...
  getDateTimeStr(): string {
    return toDateTime(this.f['scheduledDateTime'].value).toFormat(this.dateTimeFormat);
  }
  setCurrentDateTime(dateStr: string) {
    this.f['scheduledDateTime'].setValue(toDateTime(dateStr, Constants.dateTimeFormat).toJSDate());
  ...
  private copyChildData(): Map<FunctionScheduleComponent, User[]> {
    GenerateSchedulesComponent.functions2SchedulesMap.clear();
    this.fComponents.forEach((element) => {
      GenerateSchedulesComponent.functions2SchedulesMap.set(
        element,
        element.accounts4DateAndFunction,
      );
    });
  ...
  private setCopyPasteButtons() {
    ...
    else this.f['information'].setValue('There is data in a buffer');
