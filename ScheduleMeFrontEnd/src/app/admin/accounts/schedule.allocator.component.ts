...
import { Account, Role } from 'src/app/entities';
import { Schedule } from 'src/app/entities/schedule';
import { SchedulePoolElement } from 'src/app/entities/schedulepoolelement';
...
export class ScheduleAllocatorComponent implements OnInit, AfterViewInit {
        ...
        this.accountService
          ...
          .subscribe({
            next: (value) => {
              ...
              this.uniqueTasks = [
                ...new Set(
                  account.userFunctions.slice().map((f) => {
                    return f.userFunction;
                  }),
                ),
              ];
              if (this.uniqueTasks.length > 0) {
                this.f['function'].setValue(this.uniqueTasks[0]);
              }

              this.setupGroupTaskCtrl();

              this.f['scheduledDate'].setValue(new Date());
              if (this.userFunctions.length > 0) {
                this.f['function'].setValue(this.userFunctions[0].userFunction);
              }
  ...
  onAddSchedule() {
    ...
    this.isAdding = true;
    this.accountService
      .addSchedule(this.account.id, newSchedule)
      .pipe(first())
      .subscribe({
        next: (account) => {
          this.account = account;
  ...
  onDeleteSchedule(rowIndex: string, schedule2Delete: Schedule) {
    // rowIndex is table index
    // reset alerts on submit
    this.alertService.clear();

    const modalRef = this.modalService.open(NgbdModalConfirmComponent);
    modalRef.componentInstance.titleStr = 'Schedule Deletion';
    modalRef.componentInstance.bodyQuestionStr =
      'Are you sure you want to delete Schedule profile?';
  ...
  private setupGroupTaskCtrl() {
    ...
    this.uniqueGroups = [
      ...new Set(
        this.account.userFunctions
          .slice()
          .filter((f) => {
            return f.isGroup && f.userFunction == this.f['function'].value;
          })
          .map((f) => {
            return f.group;
          }),
    ...
    if (this.uniqueGroups.length > 0) {
      this.f['groupTask'].setValue(this.uniqueGroups[0]);
    }
  }
  ...
  private selectRow(schedule: Schedule) {
    ...
    if (!schedule.deleting) {
      ...
      this.setupGroupTaskCtrl();
      this.f['groupTask'].setValue(schedule.scheduleGroup);
    }
  }
  ...
  private isSameSchedule(s1: Schedule, s2: Schedule): boolean {
    ...
    return (
      s1.date == s2.date &&
      s1.userFunction == s2.userFunction &&
      s1.scheduleGroup == s2.scheduleGroup
    );
  }
  ...
  get isGroupTaskSelected() {
    ...
    for (let index = 0; index < this.agentTaskConfigs().length; index++) {
      ...
      if (possibleTask.agentTaskStr === this.f['function'].value) {
