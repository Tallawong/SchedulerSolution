...
import { ScheduleDateTimes } from 'src/app/entities/scheduledatetimes';
...
export class MainSchedulerComponent {
    ...
    this.accountService
      ...
      .subscribe({
        next: (value: ScheduleDateTimes) => {
          const oldSelected = this.futureScheduleDates().find(function (item) {
  ...
  isSameScheduleDateTime(s1: ScheduleDateTime, s2: ScheduleDateTime): boolean {
    console.assert(
      s1 != null && s2 != null,
      'One or both of the schedule date-time slots is/are null',
    );
