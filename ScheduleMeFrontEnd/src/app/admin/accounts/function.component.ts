...
export class FunctionComponent implements OnInit, AfterViewInit {
  ...
  currentSelectedContact: Task | null = null;
  lastSelectedContact: Task | null = null;
          ...
          this.accountService
            ...
            .subscribe({
              next: (value) => {
                ...
                this.f['function'].setValue(this.possibleTasks[0]?.agentTaskStr ?? '');
  ...
  groupValidator(control: FormControl): { [s: string]: boolean } | null {
  ...
  private executeAddTask() {
    ...
    console.assert(
      existing.length <= 1,
      'We have double ' + currentValue + ' task for the same user: ' + this.account.email,
    );
        ...
        next: (accounts) => {
          if (accounts != null && accounts.length > 0) {
            ...
            modalRef.componentInstance.bodyQuestionStr =
              "The following existing members of group agent '" +
              task.group +
  ...
  isSameTask(t1: Task, t2: Task): boolean {
    ...
    return (
      t1.userFunction == t2.userFunction &&
      t1.group == t2.group &&
