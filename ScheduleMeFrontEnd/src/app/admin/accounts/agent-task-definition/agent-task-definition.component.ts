...
export class AgentTaskDefinitionComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<AgentTaskConfig> = new MatTableDataSource<AgentTaskConfig>([]);
  ...
  ngOnInit(): void {
    ...
    this.f['agentTaskName'].valueChanges.subscribe({
      next: (value) => {
        console.log(value);
        if (value.length <= 0) this.f['isGroupBox'].disable();
        else this.f['isGroupBox'].enable();
      },
      complete: () => {
  ...
  onAddUpdateTask() {
    ...
    this.accountService
      .updateAgentTaskConfig(element2Update.agentTaskStr, element2Update)
      .subscribe({
  ...
  onDeleteAgentTaskConfig($event: MouseEvent, task: AgentTaskConfig) {
    ...
    modalRef.componentInstance.bodyQuestionStr =
      'Are you sure you want to delete agent task profile?';
  ...
  private selectRow(slot: AgentTaskConfig) {
    ...
    if (!slot.isDeleting) {
      this.form.patchValue({
        agentTaskName: slot.agentTaskStr,
        isGroupBox: slot.isGroup,
      });
    }
  }
