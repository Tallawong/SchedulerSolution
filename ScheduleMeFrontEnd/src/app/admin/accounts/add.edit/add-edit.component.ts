...
export class AddEditComponent implements OnInit, AfterViewInit {
  ...
  async ngOnInit() {
    ...
    if (!this.isAddMode) {
      ...
      //             this.form.get('dob').setValue(toDateTime(this.account.dob, Constants.dateFormat));
      //         }, complete: () => {
      //             this.isLoaded = true;
      //         },
      //         error: error => {
      ...
      try {
        ...
        this.form.patchValue({ dob: toDateTime(this.account.dob, Constants.dateFormat) });
        this.isLoaded = true;
      } catch (error) {
        console.error(error);
      }
      ...
      /* firstValueFrom(observable).then((value) => {
                ...
                this.account = value; // initial account
                this.form.patchValue(value);
                this.form.get('dob').setValue(toDateTime(this.account.dob));
                this.isLoaded = true;
            }).catch((error) => {
                console.error(error);
            });*/
  ...
  private createAccount() {
    ...
    account.dob = toDateTime(this.f['dob'].value).toFormat(Constants.dateFormat);
