  ...
  template: `
    <div class="fp-overlay">
      <div class="fp-modal">
        ...
        @if (!success) {
          <form (ngSubmit)="onSubmit()" #resetForm="ngForm" class="fp-form">
            @if (!token) {
              <div class="fp-error">Invalid or missing reset token.</div>
            }
            ...
            @if (token) {
              ...
              @if (passwordCtrl.touched && passwordCtrl.errors?.['minlength']) {
                <div class="fp-field-error">Password must be at least 6 characters long.</div>
              }
...
export class ResetPasswordComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if ((this.model.confirmPassword || '').length < 6) {
      this.error = 'Password must be at least 6 characters long';
      return;
    }
